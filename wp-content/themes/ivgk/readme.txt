=== VW Travel ===
Contributors: VWthemes
Tags: left-sidebar, right-sidebar, one-column, two-columns, three-columns, four-columns, grid-layout, custom-colors, custom-background, custom-logo, custom-menu, custom-header, editor-style, featured-images, footer-widgets, sticky-post, full-width-template, theme-options, translation-ready, threaded-comments, post-formats, rtl-language-support, blog, portfolio, e-commerce
Requires at least: 4.8
Tested up to: 5.1.1
Requires PHP: 7.2.14
Stable tag: 0.3
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

VW travel is a good name when it comes to the themes related to the travel business or any kind of international agency related to travel. It is also good and highly applicable for the tour planners as well as the tourist guides and a nice option for the travel and adventure blogs as well as the travel magazines. 

== Description ==

VW travel is a good name when it comes to the themes related to the travel business or any kind of international agency related to travel. It is also good and highly applicable for the tour planners as well as the tourist guides and a nice option for the travel and adventure blogs as well as the travel magazines. VW Travel is a fine theme for the hotels as well as airline agencies and the credit goes to its multiple features of relevance and these include retina ready, user friendliness, CTA, Bootstrap, customization options and above all this  theme has the optimised codes a well as the faster page load time. It has customization options and because of its core multipurpose nature, it is credible not only for the travel related business but also for any type of businesses related to the tourist industry. It is a translation ready theme and this feature is very special for the travel industry because the very word travel means moving across the globe and interacting with the different languages, cultures and civilizations. VW travel is good for tourist agencies, tour operators, travel guides, photographic agencies, travel diaries, vacation, airlines, hotels, lifestyle, technology, traveling or journey blog, fashion, and a lot more.

== Changelog ==

= 0.1 =
* Initial version released.

= 0.2 =
* Removed Holiday Tag

= 0.3 =
* Add post formats
* Add footer layout
* Add width layout option
* Add Show / hide Author, comment and post date option
* Add top to scroll with alignment option
* Add logo resizer
* Add Global color option
* Add slider content layout option
* Add slider excerpt length option
* Add slider image opacity option

== Resources ==

VW Travel WordPress Theme, Copyright 2019 VWthemes
VW Travel is distributed under the terms of the GNU GPL.

Theme is Built using the following resource bundles.

* CSS bootstrap.css
-- Copyright 2011-2018 The Bootstrap Authors
-- https://github.com/twbs/bootstrap/blob/master/LICENSE
    
* JS bootstrap.js
-- Copyright 2011-2018 The Bootstrap Authors (https://github.com/twbs/bootstrap/graphs/contributors)
-- https://github.com/twbs/bootstrap/blob/master/LICENSE

* Free to use and abuse under the MIT license.
-- http://www.opensource.org/licenses/mit-license.php
-- font-awesome.css and fonts folder
Font Awesome 5.0.0 by @davegandy - http://fontawesome.io - @fontawesome
-- License - http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)

* Customizer Pro, Copyright 2016 © Justin Tadlock.
License: All code, unless otherwise noted, is licensed under the GNU GPL, version 2 or later.
Source: https://github.com/justintadlock/trt-customizer-pro

* Theme Typography, Copyright 2016 © Justin Tadlock.
License: All code, unless otherwise noted, is licensed under the GNU GPL, version 2 or later.
Source: https://github.com/justintadlock/customizer-typography

* Stocksnap Images, 
License: CC0 1.0 Universal (CC0 1.0)
Source: https://stocksnap.io/license

Header Image, Copyright Andrew Ruiz
License: CC0 1.0 Universal (CC0 1.0)
Source: https://stocksnap.io/photo/L5HH6DFTYS

Destination Image, Copyright Johannes Plenio
License: CC0 1.0 Universal (CC0 1.0)
Source: https://stocksnap.io/photo/VOVU4FRFKD

Destination Image, Copyright Margaret Barley
License: CC0 1.0 Universal (CC0 1.0)
Source: https://stocksnap.io/photo/2F57DC2687

Destination Image, Copyright YesManPro
License: CC0 1.0 Universal (CC0 1.0)
Source: https://stocksnap.io/photo/ISAJW2MIXD

* All the icons taken from genericons licensed under GPL License.
http://genericons.com/

== Theme Documentation ==
Documentation : https://www.vwthemesdemo.com/docs/free-vw-travel/